...
import { useEssayHandleLike, useEssayLikeList, useEssayList } from '@/api/essay'
import { EssayLikeType, EssayType } from '@/api/essay/type'
...
const Essay = () => {
  ...
  // 点赞参数
  const [likeParams, setLikeParams] = useState<EssayLikeType | null>(null)
  // 回到顶部
  useEffect(() => {
    if (myRef.current) {
      window.scroll({
  ...
  const {
    ...
    hasMore,
    loadMore,
    mutate: essayListMutate
  } = useEssayList(10, '')
  const essayListSource = isEssayListFetched && essayList ? essayList : ''

  // 点赞事件
  const { eassyHandleLike } = useEssayHandleLike(likeParams)
  // 点赞
  const handleClickLike = async (essayId: string) => {
    // 获取id 根据id判断效果是否显示
    setIsLikeId(essayId)
    setEssayId((prevState) => [essayId, ...prevState])
    setTimeout(() => {
      setIsLikeId('')
    }, 500)
    const likeParams = {
  ...
  // 随笔点赞列表
  const { eassyLikeList, isEassyLikeListFetched } = useEssayLikeList()
  const likeListSource =
    isEassyLikeListFetched && eassyLikeList && eassyLikeList ? eassyLikeList.data : ''
  // 匹配已点赞随笔
  useEffect(() => {
    if (isEassyLikeListFetched && isEssayListFetched) {
      const matchedObjects = []
      for (const obj of likeListSource) {
        const essayId = obj.essayId
        const matchedObj =
          essayList && essayList.find((item) => obj.userId === clientId && item._id === essayId)
        if (matchedObj) {
          matchedObjects.push(matchedObj)
        }
      }
      const essayArr = matchedObjects.map((item) => item._id)

      if (isLikeId) {
        essayArr.push(isLikeId)
      }
      setEssayId(essayArr)
    }
  }, [isEassyLikeListFetched, isEssayListFetched, hasMore, clientId])
  return (
    <div className="w-1200 mx-auto  lg:mx-5 lg:w-full sm:w-full" ref={myRef}>
      ...
      <div className="w-1000  mt-10 pt-5 pb-5 mx-auto bg-base-100 rounded-2xl  lg:w-full sm:w-full">
        ...
        <InfiniteScroll
          // 数据长度
          dataLength={essayListSource && essayListSource.length}
          next={loadMore}
          // 是否还有可加载滴数据
          hasMore={hasMore}
          // 加载提示信息
          ...
          // 加载结束提示信息
          endMessage={
            <div className="divider text-xl mt-10" style={{ userSelect: 'none' }}>
              我也是有底线的~
            </div>
          }
        >
          {essayListSource &&
            essayListSource.map((item, index) => {
              return (
                <div
              ...
              "
                  >
                    <div>
                      <div
                        className="font-style mx-2 mb-2 text-lg"
                      ...
                      {item.cover !== undefined ? (
                        <div className="ml-2 flex flex-row flex-wrap">
                          {item.cover.map((cover: EssayType, index: number) => (
                            <Image
                              src={cover.thumbUrl}
                              key={index}
                              width={190}
                    ...
                    >
                      ...
                      <div className="text-base flex justify-end mt-1 pr-2">
                        ...
                        >
                          {essayId?.includes(item._id) ? (
                            <svg
                              className="icon w-7 h-7 mb-1  mr-[2px]"
