...
const Affiche = (props: any) => {
  ...
  return (
    <>
      <div className="bg-base-100 mb-5 h-auto mx-auto  pb-3 rounded-2xl transition duration-500 ease-in-out transform hover:scale-x-[105%] hover:scale-y-[110%] cursor-pointer">
      <p
        ...
        <Carousel autoplay autoplaySpeed={5000} dotPosition="left" dots={false} className='px-3 h-20'>
      {
            list.map((item: DataType) => (
              <p className='pt-[5px] h-20  text-base overflow-hidden'>
                <LinesEllipsis
                  text={item.content}
                  maxLine="3"
                />
