    ...
    <>
            ...
            weather.map((item: any) => {
              return (
                <p className='flex mr-[10%]'>
                  <span><IconFont iconName='icon-weizhi-xianxing' className='text-[var(--bgcolor-navbar-click)] text-[20px] ml-5'></IconFont></span>
                  <span>&nbsp;{item.location.name} {item.now.text} {item.now.temperature}°C</span>
                </p>
              )
            })
          }
      ...
      <div className="hidden lg:block lg:relative top-2 lg:h-24 lg:mx-auto   lg:rounded-xl lg:bg-base-100 lg:w-[calc(100%-30px)] sm:w-[calc(100%-45px)]">
        ...
        <Carousel autoplay autoplaySpeed={5000} dotPosition="left" dots={false} className='lg:px-3'>
          {
            list.map((item: DataType) => (
              <p className='lg:pt-[5px] lg:h-20  lg:text-base lg:text-[var(--color-font-color)] lg:overflow-y-hidden' >
                <LinesEllipsis
                  text={item.content}
                  maxLine="3"
                />
