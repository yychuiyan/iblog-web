...
import { useArticleList, useHandleLike, useLikeList } from '@/api/articles'
import { ArticleType, ArticleTypeResponse, HandleLikeType } from '@/api/articles/type'
...
const Content = () => {
  ...
  // 分类
  const [categorySwitch, setCategorySwitch] = useState('')
  // 标签
  const [tagsSwitch, setTagsSwitch] = useState('')
  // 登录数据
  // 登录状态
  // 随笔ID存储的数组
  ...
  const {
    ...
    Boolean(tag) || Boolean(category) ? 1 : currentPage,
    pageCount,
    1,
    1,
  ...
  const articleListSource =
    isArticleListFetched && articleList && articleList.data ? articleList.data : ''
  const articleListData =
    isArticleListFetched && articleList && articleList.data ? articleList.data.data : ''

  const { totalCount, page, pageSize } = articleListSource as ArticleTypeResponse
  // 点赞
  ...
  // 匹配已点赞文章
  useEffect(() => {
    if (isLikeListFetched && isArticleListFetched) {
      const matchedObjects = []
      for (const obj of likeListSource as ArticleType[]) {
        const articleId = obj.articleId
        const matchedObj =
          articleListData &&
          articleListData.find((item) => obj.userId === clientId && item._id === articleId)
        if (matchedObj) {
  ...
  }, [isArticleListFetched, isLikeListFetched, currentPage, clientId])
  ...
  const handleClickLike = (articleId: string) => {
    ...
    const articleFind = articleListData && articleListData?.find((item) => item._id === articleId)
  ...
  const onChangePage = (page: number, pageSize: number) => {
    ...
    Boolean(c) === false ? (category = '') : (category = c)
    Boolean(t) === false ? (tag = '') : (tag = t)
    setCategorySwitch(category)
    setTagsSwitch(tag)
    setPageCount(pageSize)
    setCurrentPage(page)
  }
  const handleArticle = (id: string) => {
    history(`/article/detail/${id}`)
  }
  const handleTags = (name: string) => {
    history(`/tags?t=${name}`)
  }
  const handleCategory = (name: string) => {
    history(`/category?c=${name}`)
  }
  return (
